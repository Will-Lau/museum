...
public class QuestionSetController {
    ...
    @ApiOperation(value = "创建题集", notes = "输入类别和难度及token，组卷\r\nnum为题目数量，返回值不包括答案和解析", tags = "QuestionSet")
    ...
    @ApiOperation(value = "删除题集", notes = "输入qsId和token", tags = "QuestionSet")
    ...
    @ApiOperation(value = "放弃做一个题集", notes = "输入qsId和token", tags = "QuestionSet")
    ...
    @ApiOperation(value = "获得题集", notes = "输入qId和token，做了的题会给出结果、答案和解析", tags = "QuestionSet")
    ...
    @ApiOperation(value = "获得做题进度", notes = "输入token，获得进度", tags = "QuestionSet")
    ...
    @LoginRequired
    @GetMapping("/getMaxAccuracy")
    @ApiOperation(value = "获得各个degree最高正确率", notes = "获得各个degree最高正确率度", tags = "QuestionSet")
    public List<MaxAccuarcyByDegree> getMaxAccuracy(@RequestHeader String token,String kind){
        String userId= method.getUseridByToken(token);
        if(userId==null) return null;
        return questionSetService.getMaxAccuracy(userId,kind);
    }
